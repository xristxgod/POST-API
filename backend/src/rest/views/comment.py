...
async def add_comment(body: ModComment, db: Manager = Depends(get_comment_manager)):
    return await db.add(body=await MiddlewareUtils.middleware(body=body, user=User, post=Post))
...
async def update_comment(body: ModComment, comment_id: int, db: Manager = Depends(get_comment_manager)):
    return await db.update(comment_id, body=await MiddlewareUtils.middleware(body=body, user=None, post=None))
